...
<div id="jb-container">
		...
		function initMap() {
			...
			for (i = 0; i < result.length; i++) {
				marker = new google.maps.Marker({
					...
					//label : result[i].cate,
					position : new google.maps.LatLng(result[i].lat,
							result[i].lng),
					map : map
				...
				google.maps.event.addListener(marker, 'click', (function(
						marker, i) {
					return function() {
						infowindow.setContent(result[i].guname + "구<br>"
								+ "<b>허위매물 신고 건수 : <b>" + result[i].totalaccuse
								+ "<br>" + "<b>피신고 중개인 수 : <b>"
								+ result[i].totalagent + "<br>"
								//		 													+ "<br><button type='button' id = 'insertBtn"
								//		 													+ i
								//		 													+ "' onclick='addIndex("
								//		 													+ i
								//		 													+ ");'> 장소 추가 </button>"
								+ ' <a href="/accuselist?city=seoul&gu='
								+ result[i].guname
								+ '"><p>해당 지역 신고글 자세히 보러 가기</p></a>');
	...
	<div id="jb-sidebar">
		...
		<div id="newsbar">
			<b>최신 뉴스</b> <a href="http://www.donga.com/news/List/Economy/RE"
				target="_blank">더보기</a>
					...
					<c:forEach items="${ notice }" var="i">
						<tr>
							<td style="width: 400px"><a
								href="/main/community/board/view?idx=${i.idx }">${i.title }
									<c:if test="${i.commentCnt ne 0 }">[${i.commentCnt }]</c:if>
							</a></td>
							<td style="float: right"><span style="float: right;"><font
									style="color: grey">${i.regDate }</font></span></td>
						<tr>
					</c:forEach>
				</table>
			</div>
	...
	<div id="servicecenter">
		<a href="/serviceCenterView"><img alt="고객센터"
			src="/resources/img/img_SC2.png" style="width: 10%;"></a>
	</div>
</div>
